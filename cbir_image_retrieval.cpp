...
int main() {
    ...
    // Display least similar pictures
    cout << "Least similar pictures:" << endl;
    int count = 0;
    for (const auto& result : results) {
        cout << result.second << " (Similarity: " << result.first << ")" << endl;
        if (count >= 5) break;
        Mat image = imread(DIRECTORY_PATH + "/" + result.second);
        imshow("Least Similar " + to_string(count + 1), image);
        count++;
    }

    // Sort results by similarity in descending order again
    sort(results.begin(), results.end(), [](const pair<double, string>& a, const pair<double, string>& b) {
        return a.first > b.first; // Sort in descending order
    });

    // Display top 5 most similar pictures
    cout << "\nTop 5 most similar pictures:" << endl;
    count = 0;
    for (const auto& result : results) {
        if (count >= 5) break;
        cout << result.second << " (Similarity: " << result.first << ")" << endl;
        Mat image = imread(DIRECTORY_PATH + "/" + result.second);
        imshow("Most Similar " + to_string(count + 1), image);
        count++;
    }
